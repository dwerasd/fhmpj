...
void StopChangeRegOnBP(int DebugRegNR)
{
	...
	DbgPrint("DHMKernel:StopChangeRegOnBP %d\n",DebugRegNR);
...
*/
	...
	if ( (ULONG)PsGetCurrentProcess() == (ULONG)DebuggedProcessPEPROCESS ) {
		...
		if ( iInt == 1 ) {
			...
			if (ChangeRegistersOnBP) {
				...
				for ( i = 0 ; i < 4 ; i++ ) {
					if (ChangeRegs[i].Active) {						
                        if (((DR_7.L0) && (DR_6.B0) && (DR_0==ChangeRegs[i].BreakAddress) ) ||
							...
							PEFLAGS x = (PEFLAGS)(&Stacklocation[2]);
			...
			if (((DR_7.L0) && (DR_6.B0) && (DR_0>=DebuggedAddress) && (DR_0<=DebuggedAddress+DebuggedAddressLength)) ||
